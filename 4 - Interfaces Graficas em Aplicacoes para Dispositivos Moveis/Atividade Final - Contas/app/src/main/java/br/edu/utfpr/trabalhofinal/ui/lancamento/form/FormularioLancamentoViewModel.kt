...
class FormularioLancamentoViewModel(
    ...
    private fun validarValor(valor: String): Int {
        if (valor.isBlank()) return R.string.valor_obrigatorio
        return if (parseValor(valor) == null) R.string.valor_invalido else 0
    }
    ...
    fun salvarLancamento() {
        if (formularioValido()) {
            val valorParseado = parseValor(state.valor.valor)
            if (valorParseado == null) {
                state = state.copy(
                    valor = state.valor.copy(codigoMensagemErro = R.string.valor_invalido)
                )
                return
            }
            state = state.copy(
    ...
    private fun parseValor(valor: String): BigDecimal? {
        val valorNormalizado = valor.trim().replace(',', '.')
