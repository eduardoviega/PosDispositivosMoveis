...
private val CorDespesa = Color(0xFFCF5355)
private val CorReceita = Color(0xFF00984E)
...
private fun List(
    ...
    fun valorComSinal(lacamento: Lancamento): BigDecimal {
        if (lacamento.tipo == TipoLancamentoEnum.DESPESA) return lacamento.valor.negate()
        return lacamento.valor
    }

    LazyColumn(modifier = modifier) {
        items(lancamentos) { lancamento ->
            val corItem = if (lancamento.tipo == TipoLancamentoEnum.DESPESA) CorDespesa else CorReceita
            ...
            ListItem(
                ...
                supportingContent = {
                    Row(modifier = Modifier.fillMaxWidth()) {
                        ...
                        Text(
                            text = valorComSinal(lancamento).formatar(),
                            color = corItem
                        )
                    }
                },
                leadingContent = {
                    val descricao = if (lancamento.paga) R.string.lancamento_pago else R.string.lancamento_pendente
                    Icon(
                        imageVector = if (lancamento.paga) Icons.Filled.ThumbUp else Icons.Filled.ThumbDownOffAlt,
                        contentDescription = stringResource(descricao),
                        tint = corItem
                    )
                }
            )
...
private fun BottomBar(
    ...
    fun corPorValor(valor: BigDecimal): Color {
        return if (valor < BigDecimal.ZERO) CorDespesa else CorReceita
    }
    ...
    Column(
        ...
        Totalizador(
            ...
            textColor = corPorValor(lancamentos.calcularSaldo())
        )
        Totalizador(
            modifier = Modifier.padding(bottom = 20.dp),
            titulo = stringResource(R.string.previsao),
            valor = lancamentos.calcularProjecao(),
            textColor = corPorValor(lancamentos.calcularProjecao())
        )
    }
}
